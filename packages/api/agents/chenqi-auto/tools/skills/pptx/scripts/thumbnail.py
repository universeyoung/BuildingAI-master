...
def _soffice_missing_message(pptx_path, exists: bool = False) -> str:
    ...
    return (
        ...
        "  1. If you are in the html2pptx workflow and still have the source HTML\n"
        ...
        "     The screenshots are a 1:1 preview of the .pptx because the .pptx is\n"
        "     generated from those same HTML files.\n\n"
        "  2. For ANY .pptx (no HTML required), get a structured plain-text\n"
        "     'thumbnail grid' that lists every shape's bbox / font / color /\n"
        "     text plus inventory.py's overflow + overlap warnings:\n"
        f"         python scripts/inventory_textgrid.py {pptx_path} \\\n"
        f"             -o {Path(pptx_path).stem}_textgrid.md\n"
        "     This loses pixel fidelity but is enough to catch overflow,\n"
        "     overlap, and obvious layout issues, and to read a template's\n"
        "     skeleton before re-creating it.\n\n"
